...
class Adr(Player):
    ...
    def _predict_sequence_after_flop(self, game_state: list[int]) -> float:
        """Predicts the probability of forming a straight based on 4 cards after the flop."""
        community_card_indices = game_state[2:6]  # First 4 community cards
        community_cards = [card_from_index(idx) for idx in community_card_indices if card_from_index(idx)]

        if len(community_cards) < 4:
            return 0.0  # Not enough cards for sequence prediction

        # Extract numerical ranks
        ranks = sorted([card.rank.value for card in community_cards])

        # Check for gaps to determine sequence potential
        gaps = sum(1 for i in range(len(ranks) - 1) if ranks[i + 1] - ranks[i] > 1)

        # Define probability based on gaps
        if gaps == 0:
            return 0.90  # Strong chance of a straight
        elif gaps == 1:
            return 0.70  # One card needed to complete a straight
        elif gaps == 2:
            return 0.40  # Less likely, but possible
        else:
            return 0.10  # Very low chance
